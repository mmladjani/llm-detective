...
Use the native LLM investigator on model-inference cases 011–013 to explore how an
...
1. Start the local server with `python app.py serve` and open
   `http://127.0.0.1:8000`.
2. Choose case 011, leave **LLM agent** selected, and click **Start investigation**.
...
Live comparison defaults to model cases 011–013. Its deterministic comparator uses
fixed collection, uniform weights and lexical matching; it is not the web UI's
legacy checklist. Cases 001–010 and the custom builder use authored evidence, so
...
In the main view, choose case 008, 009 or 010 from the **Human-in-the-loop** group.
...
The agent may choose other evidence instead. These cases use authored weights;
the exercise demonstrates human participation, not model-owned evidence inference.
